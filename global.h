...
void WriteConfig(){
  ...
  // Application Settings here... from EEPROM 192 up to 511 (0 - 511)
  EEPROM.write(192, config.density);
  EEPROM.write(202, config.fading_step);
  EEPROM.write(212, config.game);
  EEPROM.commit();
}
...
boolean ReadConfig(){
  ...
  if (EEPROM.read(0) == 'C' && EEPROM.read(1) == 'F'  && EEPROM.read(2) == 'G' )
  {
    ...
    config.density = EEPROM.read(192);
    config.fading_step = EEPROM.read(202);
    config.game = EEPROM.read(212);
    return true;
  }
  else
  {
